<template>
	<view>
		<view class="common-page-head-title">view</view>
		<view class="uni-padding-wrap uni-common-mt">
			<view class="uni-hello-text"> Flex是Flexible Box的缩写，意为“弹性布局”，用来为盒状模型提供最大的灵活性。当设置display:
				flex后，继续给view等容器组件设置flex-direction:
				row或column，就可以在该容器内按行或列排布子组件。uni-app推荐使用flex布局。因为flex布局有利于跨更多平台，尤其是采用原生渲染的平台。 </view>
			<view class="uni-title uni-common-mt"> flex-direction: row <text>\n横向布局</text>
			</view>
			...
			<view class="uni-title uni-common-mt"> flex-direction: column <text>\n纵向布局</text>
			</view>
			...
			<view class="uni-title uni-common-mt"> 更多布局示例 <text>\nflex布局演示</text>
			</view>
			<view>
			...
			<view class="uni-title uni-common-mt"> 组合示例 <text>\nflex布局演示</text>
			</view>
			<view class="uni-flex uni-row">
				<view class="text uni-flex"
				...
				<view class="uni-flex uni-column"
					...
					<view class="text" style="height: 120rpx;text-align: left;padding-left: 20rpx;padding-top: 10rpx;"> 文字居左，留出左间距
					</view>
					<view class="uni-flex uni-row">
...
<script>
    ...
    methods:{
		...
		onLoad() {
			setTimeout(() => {
				db.collection('a-model').get().then((res) => {
					console.log("res", res);
				})
			}, 1000)
		}
