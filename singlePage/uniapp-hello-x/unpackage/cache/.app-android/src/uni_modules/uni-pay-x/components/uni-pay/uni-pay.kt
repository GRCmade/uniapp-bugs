...
open class GenUniModulesUniPayXComponentsUniPayUniPay : VueComponent {
    constructor(__ins: ComponentInternalInstance) : super(__ins) {
        onMounted(fun() {
            var insideData: UTSJSONObject = let {
                object : UTSJSONObject(UTSSourceMapPosition("insideData", "uni_modules/uni-pay-x/components/uni-pay/uni-pay.uvue", 223, 8)) {
    ...
    override fun `$initMethods`() {
        this.open = fun(options: UTSJSONObject): UTSPromise<Unit> {
            suspend fun async(): Unit {
                ...
                if (provider != null && provider != "") {
                    ...
                    this.currentProviders = UTSAndroid.consoleDebugError(JSON.parse(JSON.stringify(this.originalRroviders)), " at uni_modules/uni-pay-x/components/uni-pay/uni-pay.uvue:244") as UTSArray<String>;
                }
        ...
        this.createOrder = fun(data: UTSJSONObject): UTSPromise<Unit> {
            suspend fun async(): Unit {
                ...
                if (options["provider"] == "appleiap") {
                    uni_showModal(ShowModalOptions(title = "提示", content = "uni-app x 暂不支持苹果内购", showCancel = false));
                    return;
                }
                if (options["provider"] == "wxpay") {
                }
                var createOrderData: UTSJSONObject = object : UTSJSONObject(UTSSourceMapPosition("createOrderData", "uni_modules/uni-pay-x/components/uni-pay/uni-pay.uvue", 302, 9)) {
        ...
        this.orderPayment = fun(res: UTSJSONObject) {
            ...
            ;
            uni_requestPayment(RequestPaymentOptions(provider = res["provider"] as String, orderInfo = orderStr, success = fun(res: RequestPaymentSuccess){
                console.log(JSON.stringify(res), " at uni_modules/uni-pay-x/components/uni-pay/uni-pay.uvue:383");
                this._getOrder();
            }
            , fail = fun(err: RequestPaymentFail){
                console.log("RequestPaymentFail", JSON.stringify(err), " at uni_modules/uni-pay-x/components/uni-pay/uni-pay.uvue:387");
                ...
                if (errCode == 700713) {
                    ...
                    console.error("uni.requestPayment:fail", err, " at uni_modules/uni-pay-x/components/uni-pay/uni-pay.uvue:398");
                    this.`$emit`("fail", err);
                }
            }
            ));
