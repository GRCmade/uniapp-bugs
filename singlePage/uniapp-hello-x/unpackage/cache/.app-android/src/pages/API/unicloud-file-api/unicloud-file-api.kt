...
open class GenPagesAPIUnicloudFileApiUnicloudFileApi : BasePage {
    ...
    override fun `$initMethods`() {
        this.uploadFile = fun() {
            uni_chooseImage(ChooseImageOptions(count = 1, success = fun(res): Unit {
                ...
                uniCloud.uploadFile(UniCloudUploadFileOptions(filePath = tempFilePath, cloudPath = "test.jpg")).then(fun(res) {
                    uni_hideLoading();
                    console.log(res, " at pages/API/unicloud-file-api/unicloud-file-api.uvue:44");
            ...
            , fail = fun(err): Unit {
                console.error("chooseImage fail: ", err, " at pages/API/unicloud-file-api/unicloud-file-api.uvue:63");
            }
            ));
        }
        ;
        this.chooseAndUploadFile = fun() {
            uniCloud.chooseAndUploadFile(UniCloudChooseAndUploadFileOptions(type = "image")).then(fun(res) {
                uni_hideLoading();
                console.log(res, " at pages/API/unicloud-file-api/unicloud-file-api.uvue:72");
                uni_showModal(ShowModalOptions(content = "上传成功", showCancel = false));
            }
            ).`catch`(fun(err: Any?) {
                uni_hideLoading();
