...
open class GenPagesAPIRequestPaymentUniPayRequestPaymentUniPay : BasePage {
    constructor(__ins: ComponentInternalInstance) : super(__ins) {
        onLoad(fun(options: OnLoadOptions) {
            console.log("onLoad: ", options, " at pages/API/request-payment-uni-pay/request-payment-uni-pay.uvue:119");
        }
        , __ins);
    }
    ...
    override fun `$render`(): Any? {
        ...
        return createElementVNode("view", utsMapOf("class" to "app"), utsArrayOf(
            ...
            createElementVNode("button", utsMapOf("class" to "button", "onClick" to fun(){
                _ctx.pageTo("/uni_modules/uni-pay-x/pages/success/success?out_trade_no=test2024030501-1&order_no=test2024030501&total_fee=1&adpid=1000000001&return_url=/pages/API/request-payment-uni-pay/order-detail");
            }
            ), "支付成功页面示例", 8, utsArrayOf(
                "onClick"
            )),
            createVNode(_component_uni_pay_popup, utsMapOf("ref" to "getOrderPopupRef", "type" to "bottom"), utsMapOf("default" to withSlotCtx(fun(): UTSArray<Any> {
                return utsArrayOf(
            ...
            createVNode(_component_uni_pay, utsMapOf("ref" to "payRef", "adpid" to _ctx.adpid, "height" to "900rpx", "return-url" to "/pages/API/request-payment-uni-pay/order-detail", "logo" to "/static/logo.png", "onSuccess" to _ctx.onSuccess, "onCreate" to _ctx.onCreate, "onFail" to _ctx.onFail, "onCancel" to _ctx.onCancel), null, 8, utsArrayOf(
                "adpid",
                "onSuccess",
                "onCreate",
                "onFail",
    ...
    override fun `$initMethods`() {
        ...
        this.toPayDesk = fun() {
            ...
            this.out_trade_no = "" + this.order_no + "-1";
            var options = let {
                object : UTSJSONObject(UTSSourceMapPosition("options", "pages/API/request-payment-uni-pay/request-payment-uni-pay.uvue", 218, 9)) {
                    var total_fee = it.total_fee
                    var order_no = it.order_no
                    var out_trade_no = it.out_trade_no
                    var description = it.description
            ...
            var optionsStr = UTSAndroid.consoleDebugError(encodeURI(JSON.stringify(options)), " at pages/API/request-payment-uni-pay/request-payment-uni-pay.uvue:227");
        ...
        this.getOrder = fun(): UTSPromise<Unit> {
            suspend fun async(): Unit {
                ...
                val payInstance = this.`$refs`["payRef"] as UniPayComponentPublicInstance;
                var getOrderData: UTSJSONObject = object : UTSJSONObject(UTSSourceMapPosition("getOrderData", "pages/API/request-payment-uni-pay/request-payment-uni-pay.uvue", 245, 9)) {
        ...
        this.onCreate = fun(res: UTSJSONObject) {
            console.log("create: ", res, " at pages/API/request-payment-uni-pay/request-payment-uni-pay.uvue:347");
        }
        ;
        this.onSuccess = fun(res: UTSJSONObject) {
            console.log("success: ", res, " at pages/API/request-payment-uni-pay/request-payment-uni-pay.uvue:352");
        ...
        this.onFail = fun(err: RequestPaymentFail) {
            console.log("fail: ", err, " at pages/API/request-payment-uni-pay/request-payment-uni-pay.uvue:364");
        }
        ;
        this.onCancel = fun(err: RequestPaymentFail) {
            console.log("cancel: ", err, " at pages/API/request-payment-uni-pay/request-payment-uni-pay.uvue:369");
        }
        ;
