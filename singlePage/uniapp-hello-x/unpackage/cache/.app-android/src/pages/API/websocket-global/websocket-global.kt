...
open class GenPagesAPIWebsocketGlobalWebsocketGlobal : BasePage {
    constructor(__ins: ComponentInternalInstance) : super(__ins) {
        ...
        onUnload(fun() {
            uni_closeSocket(CloseSocketOptions(code = 1000, reason = "close reason from client", success = fun(res: Any){
                console.log("uni.closeSocket success", res, " at pages/API/websocket-global/websocket-global.uvue:52");
            }
            , fail = fun(err: Any){
                console.log("uni.closeSocket fail", err, " at pages/API/websocket-global/websocket-global.uvue:55");
            }
            ));
            uni_hideLoading();
        }
    ...
    override fun `$initMethods`() {
        this.connect = fun() {
            ...
            uni_connectSocket(ConnectSocketOptions(url = "ws://websocket.dcloud.net.cn", header = null, protocols = null, success = fun(res: Any){
                console.log("uni.connectSocket success", res, " at pages/API/websocket-global/websocket-global.uvue:79");
            }
            , fail = fun(err: Any){
                console.log("uni.connectSocket fail", err, " at pages/API/websocket-global/websocket-global.uvue:83");
            }
            ));
            uni_onSocketOpen(fun(res){
                ...
                console.log("onOpen", res, " at pages/API/websocket-global/websocket-global.uvue:95");
            }
            );
            uni_onSocketError(fun(err){
                ...
                console.log("onError", err, " at pages/API/websocket-global/websocket-global.uvue:106");
            }
            );
            uni_onSocketMessage(fun(res){
                this.msg = res.data as String;
                console.log("onMessage", res, " at pages/API/websocket-global/websocket-global.uvue:110");
            }
            );
            uni_onSocketClose(fun(res){
                this.connected = false;
                this.msg = "";
                console.log("onClose", res, " at pages/API/websocket-global/websocket-global.uvue:115");
            }
            );
        }
        ;
        this.send = fun() {
            uni_sendSocketMessage(SendSocketMessageOptions(data = "from " + this.platform + " : " + parseInt((Math.random() * 10000).toString()).toString(), success = fun(res: Any){
                console.log(res, " at pages/API/websocket-global/websocket-global.uvue:126");
            }
            , fail = fun(err: Any){
                console.log(err, " at pages/API/websocket-global/websocket-global.uvue:129");
            }
            ));
        }
        ;
        this.close = fun() {
            uni_closeSocket(CloseSocketOptions(code = 1000, reason = "close reason from client", success = fun(res: Any){
                console.log("uni.closeSocket success", res, " at pages/API/websocket-global/websocket-global.uvue:138");
            }
            , fail = fun(err: Any){
                console.log("uni.closeSocket fail", err, " at pages/API/websocket-global/websocket-global.uvue:141");
            }
            ));
        }
        ;
