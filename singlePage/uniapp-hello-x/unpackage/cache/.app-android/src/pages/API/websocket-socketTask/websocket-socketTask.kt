...
open class GenPagesAPIWebsocketSocketTaskWebsocketSocketTask : BasePage {
    constructor(__ins: ComponentInternalInstance) : super(__ins) {
        onLoad(fun(_: OnLoadOptions) {
            this.platform = uni_getSystemInfoSync().platform;
        }
        , __ins);
        onUnload(fun() {
            uni_hideLoading();
            ...
            if (task != null) {
                task.close(CloseSocketOptions(code = 1000, reason = "close reason from client", success = fun(res: Any){
                    console.log("uni.closeSocket success", res, " at pages/API/websocket-socketTask/websocket-socketTask.uvue:57");
                }
                , fail = fun(err: Any){
                    console.log("uni.closeSocket fail", err, " at pages/API/websocket-socketTask/websocket-socketTask.uvue:60");
                }
                ));
            }
        }
    ...
    override fun `$initMethods`() {
        this.connect = fun() {
            ...
            this.socketTask = uni_connectSocket(ConnectSocketOptions(url = "ws://websocket.dcloud.net.cn", success = fun(res: Any){
                console.log("uni.connectSocket success", res, " at pages/API/websocket-socketTask/websocket-socketTask.uvue:82");
            }
            , fail = fun(err: Any){
                console.log("uni.connectSocket fail", err, " at pages/API/websocket-socketTask/websocket-socketTask.uvue:86");
            }
            ));
            this.socketTask?.onOpen(fun(res: Any){
                ...
                console.log("onOpen", res, " at pages/API/websocket-socketTask/websocket-socketTask.uvue:97");
            }
            );
            this.socketTask?.onError(fun(err: Any){
                ...
                console.log("onError", err, " at pages/API/websocket-socketTask/websocket-socketTask.uvue:107");
            }
            );
            this.socketTask?.onMessage(fun(res: OnSocketMessageCallbackResult){
                this.msg = res.data as String;
                console.log("onMessage", res, " at pages/API/websocket-socketTask/websocket-socketTask.uvue:111");
            }
            );
            this.socketTask?.onClose(fun(res: Any){
                ...
                this.msg = "";
                console.log("onClose", res, " at pages/API/websocket-socketTask/websocket-socketTask.uvue:117");
            }
            );
        }
        ;
        this.send = fun() {
            val data = "from " + this.platform + " : " + parseInt(Math.random() * 10000 + "").toString();
            this.socketTask?.send(SendSocketMessageOptions(data = data, success = fun(res: Any){
                console.log(res, " at pages/API/websocket-socketTask/websocket-socketTask.uvue:129");
            }
            , fail = fun(err: Any){
                console.log(err, " at pages/API/websocket-socketTask/websocket-socketTask.uvue:132");
            }
            ));
        }
        ;
        this.close = fun() {
            this.socketTask?.close(CloseSocketOptions(code = 1000, reason = "close reason from client", success = fun(res: Any){
                console.log("uni.closeSocket success", res, " at pages/API/websocket-socketTask/websocket-socketTask.uvue:141");
            }
            , fail = fun(err: Any){
                console.log("uni.closeSocket fail", err, " at pages/API/websocket-socketTask/websocket-socketTask.uvue:144");
            }
            ));
        }
        ;
