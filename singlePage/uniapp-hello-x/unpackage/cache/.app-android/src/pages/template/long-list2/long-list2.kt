...
open class GenPagesTemplateLongList2LongList2 : BasePage {
    ...
    open var swiperList: UTSArray<SwiperViewItem4> by `$data`;
    ...
    open var swiperTabsRect: UTSArray<SwiperTabsItem3> by `$data`;
    open var nestedScrollChildId: String by `$data`;
    @Suppress("USELESS_CAST")
    override fun data(): Map<String, Any?> {
        return utsMapOf("swiperList" to utsArrayOf<SwiperViewItem4>(SwiperViewItem4(type = "UpdatedDate", name = "最新上架", id = "list-id-1", preload = true), SwiperViewItem4(type = "FreeHot", name = "免费热榜", id = "list-id-2", preload = false), SwiperViewItem4(type = "PaymentHot", name = "付费热榜", id = "list-id-3", preload = false), SwiperViewItem4(type = "HotList", name = "热门总榜", id = "list-id-4", preload = false)), "swiperIndex" to 0, "pageScrollView" to null as UniElement?, "headerHeight" to 0, "animationFinishIndex" to 0, "tabScrollView" to null as UniElement?, "indicatorNode" to null as UniElement?, "swiperWidth" to 0, "swiperTabsRect" to utsArrayOf<SwiperTabsItem3>(), "nestedScrollChildId" to "");
    }
    override fun `$initMethods`() {
        ...
        this.cacheTabItemsSize = fun() {
            ...
            tabs.forEach(fun(node){
                this.swiperTabsRect.push(SwiperTabsItem3(x = node.offsetLeft, w = node.offsetWidth));
            }
            );
        }
        ;
        ...
        this.updateTabIndicator = fun(current_index: Number, move_to_index: Number, percentage: Number) {
            ...
            val indicator_line_x = lerpNumber4(current_size.x, move_to_size.x, percentage);
            val indicator_line_w = lerpNumber4(current_size.w, move_to_size.w, percentage);
