...
open class GenPagesTemplateSwiperListSwiperList : BasePage {
    constructor(__ins: ComponentInternalInstance) : super(__ins) {
        onLoad(fun(_: OnLoadOptions) {
            run {
                ...
                while(i < 8){
                    val space = " ".repeat(i);
                    this.swiperList.push(SwiperViewItem1(title = "Tab " + space + i));
                    i++;
                }
            }
        }
    ...
    open var swiperList: UTSArray<SwiperViewItem1> by `$data`;
    ...
    open var swiperTabsRect: UTSArray<SwiperTabsItem1> by `$data`;
    @Suppress("USELESS_CAST")
    override fun data(): Map<String, Any?> {
        return utsMapOf("swiperList" to utsArrayOf<SwiperViewItem1>(), "swiperIndex" to 0, "tabScrollView" to null as UniElement?, "indicatorNode" to null as UniElement?, "animationFinishIndex" to 0, "swiperWidth" to 0, "swiperTabsRect" to utsArrayOf<SwiperTabsItem1>());
    }
    override fun `$initMethods`() {
        ...
        this.cacheTabItemsSize = fun() {
            ...
            tabs.forEach(fun(node){
                this.swiperTabsRect.push(SwiperTabsItem1(x = node.offsetLeft, w = node.offsetWidth));
            }
            );
        }
        ;
