        ...
        return createElementVNode(Fragment, null, utsArrayOf(
            ...
            createElementVNode("scroll-view", utsMapOf("style" to normalizeStyle(utsMapOf("flex" to "1")), "refresher-enabled" to true, "refresher-default-style" to "none", "onRefresherpulling" to _ctx.onRefresherpulling, "refresher-threshold" to 300, "refresher-max-drag-distance" to 299, "onScroll" to _ctx.onScroll, "rebound" to false), utsArrayOf(
                createElementVNode("view", utsMapOf("class" to "head-img-box-2", "ref" to "head-img-box-2"), utsArrayOf(
                    createElementVNode("image", utsMapOf("class" to "img", "ref" to "head-img-2", "src" to "/static/template/pull-zoom-image/head-img.jpg", "mode" to "scaleToFill"), null, 512)
                ), 512),
                ...
                createElementVNode("view", utsMapOf("slot" to "refresher"), utsArrayOf(
                    createElementVNode("view", utsMapOf("class" to "head-img-box-1"), utsArrayOf(
                        createElementVNode("image", utsMapOf("class" to "img", "ref" to "head-img-1", "src" to "/static/template/pull-zoom-image/head-img.jpg", "mode" to "scaleToFill"), null, 512)
                    ))
                ))
            ), 44, utsArrayOf(
                "onRefresherpulling",
    ...
    override fun `$initMethods`() {
        ...
        this.back = fun() {
            uni_navigateBack(NavigateBackOptions(success = fun(result) {
                console.log("navigateBack success", result.errMsg, " at pages/template/pull-zoom-image/pull-zoom-image.uvue:77");
            }
            , fail = fun(error) {
                console.log("navigateBack fail", error.errMsg, " at pages/template/pull-zoom-image/pull-zoom-image.uvue:80");
            }
            , complete = fun(result) {
                console.log("navigateBack complete", result.errMsg, " at pages/template/pull-zoom-image/pull-zoom-image.uvue:83");
            }
            ));
        }
        ;
