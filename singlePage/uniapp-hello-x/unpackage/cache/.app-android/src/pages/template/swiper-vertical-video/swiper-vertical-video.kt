...
open class GenPagesTemplateSwiperVerticalVideoSwiperVerticalVideo : BasePage {
    constructor(__ins: ComponentInternalInstance) : super(__ins) {
        ...
        onHide(fun() {
            ...
            console.log("pages-onHide", " at pages/template/swiper-vertical-video/swiper-vertical-video.uvue:101");
        ...
        , fun(current: Number, oldCurrent: Number) {
            ...
            if (Math.abs(changeNumber) == 2) {
                ...
                if (this.list.length < 3 * page + 3) {
                    var list: UTSArray<ListItem4> = this.getData();
            ...
            this.state.forEach(fun(_: String, index: Number){
                if (index === current) {
                    ...
                    this.doStop(index);
                    console.log("index:" + index + "已被执行停止", " at pages/template/swiper-vertical-video/swiper-vertical-video.uvue:82");
                }
            }
            );
        }
            ...
            createElementVNode("swiper", utsMapOf("class" to "swiper", "current" to _ctx.current, "circular" to (_ctx.index != 0), "vertical" to true, "onChange" to _ctx.onSwiperChange, "onTransition" to _ctx.onTransition), utsArrayOf(
                createElementVNode(Fragment, null, RenderHelpers.renderList(_ctx.visibleList, fun(item, i, __index, _cached): Any {
                    return createElementVNode("swiper-item", utsMapOf("class" to "swiper-item", "key" to i), utsArrayOf(
                        ...
                        , "src" to item.src, "poster" to item.poster_src, "autoplay" to false, "show-center-play-btn" to false, "loop" to true, "onPlay" to fun(){
                            _ctx.onPlay(i);
                        }
                        , "onPause" to fun(){
                            _ctx.onPause(i);
    ...
    open var list: UTSArray<ListItem4> by `$data`;
    open var visibleList: UTSArray<ListItem4> by `$data`;
    open var current: Number by `$data`;
    open var index: Number by `$data`;
    open var state: UTSArray<String> by `$data`;
    @Suppress("USELESS_CAST")
    override fun data(): Map<String, Any?> {
        return utsMapOf("\$videoContextMap" to Map<String, VideoContext>(), "list" to utsArrayOf<ListItem4>(), "visibleList" to utsArrayOf<ListItem4>(), "current" to 0 as Number, "index" to 0, "state" to utsArrayOf<String>("unPlay", "unPlay", "unPlay"));
    }
    override fun `$initMethods`() {
        ...
        this.onLoadstart = fun(index: Number) {
            console.error("onLoadstart  video" + index, " at pages/template/swiper-vertical-video/swiper-vertical-video.uvue:116");
        }
        ;
        ...
        this.doPlay = fun(index: Number) {
            console.log("doPlay  video" + index, " at pages/template/swiper-vertical-video/swiper-vertical-video.uvue:128");
            this.getVideoContext(index).play();
        }
        ;
        this.doStop = fun(index: Number) {
            console.log("doStop  video-" + index, " at pages/template/swiper-vertical-video/swiper-vertical-video.uvue:132");
        ...
        this.doPause = fun(index: Number) {
            ...
            console.log("doPause  video-" + index, " at pages/template/swiper-vertical-video/swiper-vertical-video.uvue:141");
        }
        ;
        this.onPause = fun(index: Number) {
            this.state[index] = "pause";
            console.log("onPause", index, " at pages/template/swiper-vertical-video/swiper-vertical-video.uvue:145");
        }
        ;
        this.onPlay = fun(index: Number) {
            if (this.current != index || !currentPageIsShow) {
                ...
                console.log("onPlay", index, " at pages/template/swiper-vertical-video/swiper-vertical-video.uvue:152");
            }
        }
        ;
        this.getData = fun(): UTSArray<ListItem4> {
            ...
            var list = utsArrayOf<ListItem4>();
            run {
                var i: Number = 0;
                while(i < 6){
                    var index = this.list.length + i;
                    var listItem = ListItem4(_id = "a00" + index, content = "这是第" + index + "条数据，url地址" + videoUrlList[i % 3], src = videoUrlList[i % 3], poster_src = posterSrcList[i % 3]);
                    list.push(listItem);
                    i++;
                }
            }
        ...
        this.back = fun() {
            uni_navigateBack(NavigateBackOptions(success = fun(result) {
                console.log("navigateBack success", result.errMsg, " at pages/template/swiper-vertical-video/swiper-vertical-video.uvue:191");
            }
            , fail = fun(error) {
                console.log("navigateBack fail", error.errMsg, " at pages/template/swiper-vertical-video/swiper-vertical-video.uvue:194");
            }
            , complete = fun(result) {
                console.log("navigateBack complete", result.errMsg, " at pages/template/swiper-vertical-video/swiper-vertical-video.uvue:197");
            }
            ));
        }
        ;
    ...
    open lateinit var getData: () -> UTSArray<ListItem4>;
