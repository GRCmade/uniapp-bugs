...
open class GenPagesTemplateListNewsListNews : BasePage {
    ...
    override fun `$initMethods`() {
        this.getBanner = fun() {
            var data = object : UTSJSONObject(UTSSourceMapPosition("data", "pages/template/list-news/list-news.uvue", 66, 13)) {
            ...
            , fail = fun(e){
                console.log("fail", e, " at pages/template/list-news/list-news.uvue:82");
            }
            ));
        }
        ;
        this.getList = fun() {
            ...
            uni_request<UTSArray<Item6>>(RequestOptions(url = url, method = "GET", success = fun(res){
                if (res.statusCode == 200) {
                    console.log(res, " at pages/template/list-news/list-news.uvue:100");
                    val result = res.data;
                    if (result != null) {
            ...
            , fail = fun(res){
                console.log("fail", res, " at pages/template/list-news/list-news.uvue:111");
                this.refresherTriggered = false;
            }
            ));
        }
