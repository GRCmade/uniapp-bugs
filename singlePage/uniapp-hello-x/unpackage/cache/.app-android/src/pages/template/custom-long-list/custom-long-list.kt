...
open class GenPagesTemplateCustomLongListCustomLongList : BasePage {
    ...
    override fun `$render`(): Any? {
        ...
        return createElementVNode("view", utsMapOf("style" to normalizeStyle(utsMapOf("flex" to "1", "background-color" to "aliceblue"))), utsArrayOf(
            ...
            createVNode(_component_uni_recycle_view, utsMapOf("style" to normalizeStyle(utsMapOf("flex" to "1")), "list" to _ctx.list, "onScrolltoupper" to _ctx.scrolltoupper, "onScroll" to _ctx.scroll), utsMapOf("default" to withScopedSlotCtx(fun(slotProps: Map<String, Any?>): UTSArray<Any> {
                val items = slotProps["items"];
                return utsArrayOf(
                    createElementVNode(Fragment, null, RenderHelpers.renderList((items as UTSArray<Item7>), fun(item, __key, __index, _cached): Any {
                        return createVNode(_component_uni_recycle_item, utsMapOf("class" to "item", "item" to item, "key" to item.id), utsMapOf("default" to withSlotCtx(fun(): UTSArray<Any> {
                            return utsArrayOf(
    ...
    override fun `$initMethods`() {
        this.scrolltoupper = fun() {
            console.log("scroll top upper", " at pages/template/custom-long-list/custom-long-list.uvue:43");
        }
        ;
        this.scroll = fun() {};
    }
