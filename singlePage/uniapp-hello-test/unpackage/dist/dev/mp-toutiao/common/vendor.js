...
function applyOptions$1(instance) {
  ...
  }
  {
    ctx.$callCreatedHook = function(name) {
      if (created) {
        return callHook$1(created, instance, "c");
      }
    };
  }
...
function initInjections(instance) {
  ...
  if (isArray(injectOptions)) {
    for (let i2 = 0; i2 < injectOptions.length; i2++) {
      const key = injectOptions[i2];
...
const S = true, b = "mp-toutiao", A = I(define_process_env_UNI_SECURE_NETWORK_CONFIG_default), C = b, P = I(""), T = I("[]") || [];
let O = "";
try {
  O = "__UNI__288052F";
} catch (e2) {
...
Un = class {
  constructor() {
    throw Ln({ message: `Platform ${C} is not supported by secure network` });
  }
};
    ...
    }
    {
      ...
      if (!index.__stat_uniCloud_space) {
        if (space && Object.keys(space).length !== 0) {
          ...
          index.__stat_uniCloud_space = Vs.init(spaceData);
        } else {
