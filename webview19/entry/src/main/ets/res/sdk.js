...
// ../uni-app-next/packages/uni-uts-v1/lib/javascript/lib/runtime/uts.js
...
function arrayFind(array, predicate) {
  const index3 = array.findIndex(predicate);
  if (index3 < 0) {
    return null;
  }
  return array[index3];
...
function arrayFindLast(array, predicate) {
  const index3 = array.findLastIndex(predicate);
  if (index3 < 0) {
    return null;
  }
  return array[index3];
}
function arrayAt(array, index3) {
  if (index3 < -array.length || index3 >= array.length) {
    return null;
  }
  return array.at(index3);
}
var IDENTIFIER;
...
var utsMetadataKey = IDENTIFIER.UTS_METADATA;
...
var UTSType = class _UTSType {
  ...
  static withGenerics(parent, generics, isJSONParse = false) {
    if (isJSONParse) {
      const illegalGeneric = generics.find((item) => !(item === Array || isBaseType(item) || isUnknownType(item) || isAnyType(item) || item === UTSJSONObject || item.prototype && item.prototype instanceof _UTSType));
      if (illegalGeneric) {
      ...
      return class UTSMap extends _UTSType {
        constructor(options, isJSONParse2 = false) {
          ...
          for (const key2 in options) {
            obj.set(normalizeGenericValue(key2, generics[0], isJSONParse2), normalizeGenericValue(options[key2], generics[1], isJSONParse2));
          }
          return obj;
        }
      };
  ...
  static initProps(options, metadata, isJSONParse = false) {
    ...
    for (const key2 in metadata.fields) {
      const { type, optional, jsonField } = metadata.fields[key2];
      const realKey = isJSONParse ? jsonField || key2 : key2;
      if (options[realKey] == null) {
        if (optional) {
          obj[key2] = null;
          continue;
        } else {
          throw new UTSError(`Failed to contruct type, missing required property: ${key2}`);
        }
      }
      if (isUTSType(type)) {
        obj[key2] = isJSONParse ? (
          // @ts-ignore
          new type(options[realKey], void 0, isJSONParse)
        ) : options[realKey];
      } else if (type === Array) {
        if (!Array.isArray(options[realKey])) {
          throw new UTSError(`Failed to contruct type, property ${key2} is not an array`);
        }
        obj[key2] = options[realKey];
      } else {
        obj[key2] = options[realKey];
      }
    }
...
var UTSJSONObject$1 = class UTSJSONObject2 {
  static keys(obj) {
  ...
  constructor(content = {}) {
    if (content instanceof Map) {
      content.forEach((value, key2) => {
        this[key2] = value;
      });
    } else {
      for (const key2 in content) {
        if (Object.prototype.hasOwnProperty.call(content, key2)) {
          this[key2] = content[key2];
        }
      }
    }
  ...
  get(key2) {
    return this._getValue(key2);
  }
  set(key2, value) {
    this[key2] = value;
  }
  getAny(key2, defaultValue) {
    const realDefaultValue = getRealDefaultValue(defaultValue);
    return this._getValue(key2, realDefaultValue);
  }
  getString(key2, defaultValue) {
    const realDefaultValue = getRealDefaultValue(defaultValue);
    const value = this._getValue(key2, realDefaultValue);
  ...
  getBoolean(key2, defaultValue) {
    const realDefaultValue = getRealDefaultValue(defaultValue);
    const boolean = this._getValue(key2, realDefaultValue);
  ...
  getJSON(key2, defaultValue) {
    const realDefaultValue = getRealDefaultValue(defaultValue);
    let value = this._getValue(key2, realDefaultValue);
    if (value instanceof Object) {
      return value;
    } else {
      return realDefaultValue;
  ...
  toMap() {
    ...
    for (let key2 in this) {
      map2.set(key2, this[key2]);
    }
    return map2;
  }
  forEach(callback) {
    for (let key2 in this) {
      callback(this[key2], key2);
    }
  }
};
...
function createUTSJSONObjectOrArray(obj) {
  if (Array.isArray(obj)) {
    ...
    for (const key2 in obj) {
      ...
      result[key2] = createUTSJSONObjectOrArray(value);
    }
    return result;
  }
  return obj;
...
var UTSValueIterable$1 = class UTSValueIterable2 {
};
function getGlobal() {
...
import jsRuntime from "libjsruntime.so";
...
var isRealPlainObject = (val) => {
  if (toTypeString(val) !== "[object Object]") {
    return false;
  }
  const proto = Object.getPrototypeOf(val);
  if (proto === null) {
  ...
  return proto === baseProto || proto === null;
};
...
function formatValueRound(px) {
  return Math.round(px2vp(px));
}
function formatValueCeil(px) {
...
function createStoreReference(value) {
  ...
  if (!isObject2 && !isFunction2 || value == null || value instanceof ArrayBuffer) {
    return value;
  }
  if (Array.isArray(value)) {
    return value.map((item) => createStoreReference(item));
  }
  if (isRealPlainObject(value) || globalThis.UTSJSONObject && value instanceof globalThis.UTSJSONObject) {
    const obj = {};
    for (const key2 in value) {
      obj[key2] = createStoreReference(value[key2]);
    }
    return obj;
  }
  const id = storeId++;
  ...
  return {
    __type: isFunction2 ? UTS_PROXY_TYPE.FUNCTION : UTS_PROXY_TYPE.OBJECT,
    id,
    keyInfo
  };
}
...
function normalizeArgument(arg, callbacks) {
  const isPlainObjectArg = isRealPlainObject(arg);
  if (isPlainObjectArg && arg.__type === UTS_PROXY_TYPE.FUNCTION) {
    return callbacks[arg.id];
  } else if (isPlainObjectArg) {
...
function isClass(obj) {
  return typeof obj === "function" && obj.prototype && obj.prototype.constructor === obj && !obj.prototype.constructor.toString().includes("function");
}
function _invokeStoreReference(id, method, args) {
  ...
  switch (method) {
    case "__get": {
      let prop = storeValue[args[0]];
      if (typeof prop === "function" && !isClass(prop)) {
        prop = prop.bind(storeValue);
      }
      return prop;
    }
    case "__set":
    ...
    case "__invoke": {
      const normalizedArgs = normalizeArguments(args[0], args[1]);
      return storeValue(...normalizedArgs);
    }
    case "__construct": {
      return new storeValue(...args);
    }
    default:
...
var MPBridge = {
  onNativeEventReceive(callback) {
    const appId = getCurrentMP()?.appId;
...
var UniMP = class {
  constructor(id, config) {
    ...
    this.animationType = config?.animationType ?? "slide-in-bottom";
    this.animationDuration = config?.animationDuration ?? 300;
    this.showSplashScreen = config?.showSplashScreen ?? !this.isMain;
    ...
    if (launchAppScheme) {
      launchOptions.redirectInfo.appScheme = launchAppScheme;
      launchOptions.redirectInfo.appLink = launchAppScheme;
    }
    ...
    this.startWindowBackground = this.manifest["app-harmony"]?.distribute?.splashScreens?.startWindowBackground ?? "#FFFFFF";
    this.startWindowIcon = this.manifest["app-harmony"]?.distribute?.splashScreens?.startWindowIcon;
    initUserAgentOnce(this.manifest, this.userAgent);
    const initWindow = (err, window5) => {
      if (err.code) {
        console.error(err);
        return;
      }
      ...
      this.window = window5;
      window5.setSubWindowModal(true);
      window5.setWindowLayoutFullScreen(true);
      window5.on("keyboardHeightChange", (size) => {
        appLifeCycle.emit("keyboardHeightChange", size);
      });
      try {
        window5.on("subWindowClose", () => {
          setTimeout(() => {
            this.close();
          }, 0);
          return true;
      ...
      window5.on("windowStatusChange", windowStatusChange);
      windowStatusChange(window5.getWindowStatus());
      ...
      window5.loadContentByName(TEMPLATE_PAGE_NAME, storage, (err2) => {
        if (err2.code) {
          console.error(err2);
          return;
        }
        const restore3 = setCurrentMP(this);
        this.context = window5.getUIContext();
  ...
  close() {
    ...
    window5.destroyWindow();
    webview.clear();
    delete store2[this.appId];
    restore();
    this._emitter.emit("close");
    this._emitter.off("close");
    this._closed = true;
  }
  isRunning() {
    return !this._closed;
...
function hmrOpenUniApp(appId, config) {
  const mp = openUniApp(appId, config);
...
function hmrRestartUniApp() {
  ...
  const mainMPId = currentAppId || "HBuilder";
  return hmrOpenUniApp(mainMPId, {
...
appAbilityEmitter.on(
  ...
  (want, launchParam) => {
    ...
    if (mainMP) {
      ...
      const uri2 = want?.uri;
      if (uri2) {
        mainMP.launchOptions.redirectInfo.appScheme = uri2;
        mainMP.launchOptions.redirectInfo.userAction = true;
        mainMP.launchOptions.redirectInfo.appLink = uri2;
      }
    }
  }
);
...
function isInRaw(appId) {
  appId = appId ?? getCurrentMP().appId;
...
function getEnv() {
  ...
  const USER_DATA_PATH = context.filesDir + "/" + appId;
  const TEMP_PATH = context.tempDir + "/" + appId;
  const CACHE_PATH = context.cacheDir + "/" + appId;
...
// packages/uni-mp-sdk/src/UTSHarmony/osTheme.ts
var import_tiny_emitter3 = __toESM(require_tiny_emitter());
import ConfigurationConstant from "@ohos.app.ability.ConfigurationConstant";

// packages/uni-mp-sdk/src/constants.ts
var ON_OS_THEME_CHANGE = "onOsThemeChange";
var ON_APP_THEME_CHANGE = "onAppThemeChange";

// packages/uni-mp-sdk/src/UTSHarmony/osTheme.ts
var osThemeChangeEmitter = new import_tiny_emitter3.TinyEmitter();
function colorMode2OsTheme(colorMode) {
  switch (colorMode) {
...
appAbilityEmitter.on(
  ...
  (config) => {
    ...
    if (lastColorMode !== colorMode) {
      ...
      appLifeCycle.emit(ON_OS_THEME_CHANGE, osTheme);
...
function getSafeAreaInsets(px) {
  ...
  if (currentWindow) {
    ...
    safeAreaInsets.top = px ? systemArea.topRect.height : formatValueRound(systemArea.topRect.height);
    safeAreaInsets.bottom = px ? navArea.bottomRect.height : formatValueRound(navArea.bottomRect.height);
    safeAreaInsets.keyboardHeight = px ? keyboard.bottomRect.height : formatValueRound(keyboard.bottomRect.height);
  }
...
function onSafeAreaInsetsChange(callback, px) {
  ...
  if (!safeAreasEvent) {
    safeAreasEvent = new import_tiny_emitter4.TinyEmitter();
    ...
    const safeAreaInsetsChangeCallback = function(options) {
      ...
      switch (options.type) {
        case window2.AvoidAreaType.TYPE_KEYBOARD:
          safeAreaInsets.keyboardHeight = px ? options.area.bottomRect.height : formatValueRound(options.area.bottomRect.height);
          break;
        case window2.AvoidAreaType.TYPE_SYSTEM:
          safeAreaInsets.top = px ? options.area.topRect.height : formatValueRound(options.area.topRect.height);
          break;
        case window2.AvoidAreaType.TYPE_NAVIGATION_INDICATOR:
          safeAreaInsets.bottom = px ? options.area.bottomRect.height : formatValueRound(options.area.bottomRect.height);
          break;
        default:
          break;
      }
...
function lookupContentTypeWithUri(uri2) {
  const uriArr = uri2.split(".");
  if (uriArr.length <= 1) {
    return void 0;
  }
...
var NativeEmbedNodeController = class extends NodeController {
  constructor(parent, id, info, surfaceId, embedId, elId) {
    ...
    this.renderType = defineNativeEmbedOptions.nodeRenderType ?? NodeRenderType.RENDER_TYPE_TEXTURE;
  ...
  parseOptions(info) {
    ...
    try {
      ...
      Object.keys(newOptions).forEach((key2) => {
        ...
        if (key2 === "on") {
          const on = options.on = /* @__PURE__ */ new Map();
          const events = value;
          events.forEach((key3) => {
            on.set(key3, (event) => {
              if (this.isDetached) {
...
function removeWebNode(mp, id) {
  ...
  const { store: store5, createCallback } = mpCache.node;
  const array = store5[id];
  if (array) {
    array[0]?.dispose();
    delete store5[id];
...
function getWebNode(mp, id, nodeController) {
  const { store: store5 } = cache.get(mp).node;
  let array = store5[id];
  if (!array) {
    array = store5[id] = [null, nodeController, null];
  } else {
...
var WebNodeController = class extends NodeController2 {
  ...
  aboutToDisappear() {
    const { store: store5 } = cache.get(this.mp).node;
    const array = store5[this.id];
    if (array) {
      if (array[1] === this) {
        array[1] = null;
      }
...
var WebView = class {
  constructor(src, id, style, extras) {
    ...
    this._emitter = new import_tiny_emitter5.TinyEmitter();
    ...
    this._children = new Proxy(
      {},
      {
        set: (target, prop, value) => {
          const storeId2 = prop;
          target[storeId2] = value;
          const childTaskId = `${this.id}-${value.elId}`;
          const tasks = this._childTasks[childTaskId] || [];
          if (tasks.length > 0) {
            tasks.forEach((task) => task());
            delete this._childTasks[childTaskId];
          }
          return true;
        }
      }
        ...
        invokeSync: (name, args = []) => {
          ...
          switch (name) {
            ...
            case "getSameOriginUrl": {
              const url = args[0];
              if (/^file:\/\//.test(url)) {
                return url.replace(/^file:\/\//, UniFileProxy);
              }
              if (/^https{0,1}:\/\//.test(url)) {
                const uriInstance = new uri.URI(url);
                let ext = uriInstance.clearQuery().getLastSegment().split(".")[1] || "";
                switch (ext) {
                  case "svg":
                    ext = "svg+xml";
                    break;
                }
                if (!Base64ImageMimeType.includes(ext)) {
                  ext = "jpeg";
                }
                let resBuffer = new ArrayBuffer(0);
                ...
                return new Promise((resolve) => {
                  httpRequest.requestInStream(url, {
                    ...
                    if (code === http.ResponseCode.OK) {
                      try {
                        resolve(
                          `data:image/${ext};base64,${buffer2.from(resBuffer).toString("base64")}`
                        );
                      } catch (error) {
                        resolve(url);
                      }
                  ...
                  httpRequest.on("dataReceive", (buffer6) => {
                    const newRes = new ArrayBuffer(
                      resBuffer.byteLength + buffer6.byteLength
                    );
                    const resView = new Uint8Array(newRes);
                    resView.set(new Uint8Array(resBuffer));
                    resView.set(new Uint8Array(buffer6), resBuffer.byteLength);
                    resBuffer = newRes;
                  });
                });
              }
            ...
            case "invokeNativeEmbed":
              {
                ...
                const nodeController = this.getCurrentNodeController(elId);
                if (nodeController) {
                  return nodeController?.invoke(method, subArgs);
                }
                this._registerChildTask(elId, () => {
                  const nodeController2 = this.getCurrentNodeController(elId);
                  return nodeController2?.invoke(method, subArgs);
                });
              }
              break;
            default:
    ...
    this._safeAreaInsetsChangeHandler = (res) => {
      ...
      if (typeof keyboardHeight === "number" && topView === this) {
        ...
        const style2 = this.pageState.style;
        if (mode === "adjustPan") {
   ...
   */
  _registerChildTask(elId, task) {
    if (this.getCurrentNodeController(elId)) {
      task();
    }
    const childTaskId = `${this.id}-${elId}`;
    this._childTasks[childTaskId] = this._childTasks[childTaskId] || [];
    this._childTasks[childTaskId].push(task);
  }
  ...
  close(aniClose, duration, extras) {
    ...
    this._emitter.emit("close");
    const { store: store5, stack } = cache.get(this.mp);
    delete store5[this.id];
  ...
  getSafeAreaInsets() {
    ...
    const top = Math.max(formatValueRound(safeAreaInsets.top - y), 0);
    const bottom = Math.max(
      formatValueRound(
        safeAreaInsets.bottom - (windowRect.height - (y + size.height))
      ),
      0
    );
    return {
      ...
      screenTop: Math.round(position.y),
      // webview顶部到window顶部的高度
      windowHeight: formatValueCeil(size.height),
      // webview高度
      windowWidth: formatValueCeil(size.width),
      // webview宽度
      screenWidth: formatValueCeil(windowRect.width),
      // window宽度
      screenHeight: formatValueCeil(windowRect.height)
      // window高度
    };
  }
  getShareOptions() {
  ...
  loadURL(url, additionalHttpHeaders) {
    ...
    if (additionalHttpHeaders) {
      headers = Object.entries(additionalHttpHeaders).map(([k, v]) => ({
        headerKey: k,
        headerValue: v
      }));
    }
  ...
  setStyle(styles) {
    ...
    newStyleKeys.forEach((key2) => {
      ...
      if (isPlainObject2(curStyle) && isPlainObject2(this.style[key2])) {
...
function createLaunchWebView(manifest) {
  ...
  let path = manifest.launch_path || "__uniappview.html";
...
var entryAbility = null;
function getEntryAbility() {
  return entryAbility;
}
...
var pageLifeCycle = new import_tiny_emitter6.TinyEmitter();
...
var appLifeCycle = new import_tiny_emitter6.TinyEmitter();
var globalConfig = {};
function initAbility(ability) {
...
function hmrGetUniAppWWWPath(appId) {
  return getPrivateDir(appId || "HBuilder") + "/www";
}
function hmrGetUniAppWWWPathInResfile(appId) {
  return getAbilityContext().resourceDir + "/apps/" + (appId || "HBuilder") + "/www";
}
var www_id = 0;
function releaseCurrentWgtToRunPath(wgtPath, callback) {
  const currentAppId = getCurrentMP().appId;
  if (currentAppId) {
    ...
    (async () => {
      if (fs3.accessSync(tempWgtPath)) {
        await fs3.rename(tempWgtPath, `${tempWgtPath}_${(/* @__PURE__ */ new Date()).getTime()}`);
      }
      ...
      return fs3.readText(`${tempWgtPath}/manifest.json`).then(async (fileString) => {
        ...
        if (currentAppId === "HBuilder" || currentAppId === appId) {
          ...
          if (fs3.accessSync(wgtReleasePath)) {
            await fs3.rename(wgtReleasePath, `${TEMP_PATH}/www_${www_id++}`);
          }
          if (!fs3.accessSync(wgtReleasePath)) {
          ...
          throw new Error("installWgt[rename]:fail WGT \u5B89\u88C5\u5305\u4E2D manifest.json \u6587\u4EF6\u7684 appid \u4E0D\u5339\u914D");
        }
      }).catch((e) => {
        if (e.code === 13900002) {
          throw new Error(`installWgt[rename]:fail wgt manifest.json is not found`);
        } else {
          throw new Error(`installWgt[rename]:fail ${e.message}`);
        }
      });
    }).catch((e) => {
      callback(-1, {
        message: `installWgt[decompress]:fail ${e.message}`
      });
    });
  } else {
    callback(-1, {
      message: "installWgt:fail Current appId is not found"
    });
  }
}
  ...
  addEventListener(name, callback) {
    ...
    switch (name) {
      ...
      case "uistylechange": {
        ...
        appLifeCycle.on(ON_OS_THEME_CHANGE, fn);
        mp.on("beforeClose", () => {
          appLifeCycle.off(ON_OS_THEME_CHANGE, fn);
        });
        break;
      }
      default:
...
// packages/uni-mp-sdk/src/weex/crypto.ts
import cryptoFramework from "@ohos.security.cryptoFramework";
import buffer3 from "@ohos.buffer";
var crypto = {
  getRandomValues(arrayLength) {
    const random = cryptoFramework.createRandom();
    try {
      const randData = random.generateRandomSync(arrayLength);
      if (randData !== null) {
        return buffer3.from(randData.data).toString("base64");
      } else {
        console.error("[Sync]: get rand result fail!");
      }
    } catch (error) {
      const e = error;
      console.error(`do getRandomValues failed, ${e.code}, ${e.message}`);
    }
  }
};
  ...
  registerServiceMethod("base64ToTempFilePath", (args, resolve) => {
    ...
    fs4.createStream(
      ...
      (err, stream) => {
        ...
        stream.write(
          buffer4.from(base64, "base64").buffer,
          (err2) => {
            if (err2) {
              stream.closeSync();
              return resolve(err2);
...
var isPlainObject3 = (val) => toTypeString2(val) === "[object Object]";
...
function assertType(value, type) {
  let valid;
  const expectedType = getType3(type);
  if (isSimpleType(expectedType)) {
    ...
    if (__PLATFORM__ === "app") {
      valid = value instanceof type || toRawType(value) === getType3(type);
    } else {
      valid = value instanceof type;
    }
  }
...
function hasCallback(args) {
  if (isPlainObject3(args) && [API_SUCCESS, API_FAIL, API_COMPLETE].find((cb) => isFunction(args[cb]))) {
    return true;
  }
  return false;
}
...
function formatApiArgs(args, options) {
  ...
  const formatArgs2 = options.formatArgs;
  const keys = Object.keys(formatArgs2);
  for (let i = 0; i < keys.length; i++) {
    const name = keys[i];
    const formatterOrDefaultValue = formatArgs2[name];
...
var addInterceptor = defineSyncApi(API_ADD_INTERCEPTOR, (method, interceptor) => {
  if (isString(method) && isPlainObject3(interceptor)) {
...
// ../uni-app-next/packages/uni-app-harmony/src/compiler/external-modules.json
var external_modules_default = [
  {
    type: "extapi",
...
for (const pkg of external_modules_default) {
  if (pkg.type !== "extapi") {
    continue;
  }
...
// packages/uni-mp-sdk/src/uni-console-format.js
...
function formatArrayElement(value, index3, depth) {
  const result = formatArg(value, depth);
  result.name = `${index3}`;
  return result;
}
...
// packages/uni-mp-sdk/src/uni-console.ts
var sendConsoleMessages;
...
function rewriteConsole() {
  consoleMethods.forEach((method) => {
    console[method] = (...args) => {
      ...
      if (typeof filename === "string" && atFileRegex.test(filename)) {
        args.pop();
      } else {
        filename = "";
      }
      const message = __f__(method, filename, ...args);
      sendConsoleMessages([message]);
    };
  });
}
...
var UniConsole = {
  setSendConsoleMessages(fn) {
    sendConsoleMessages = fn;
    rewriteConsole();
  },
  restoreConsole
};
setStoreValue("__UniConsole" /* __UniConsole */, UniConsole);
...
function getUIAbilityContext() {
  ...
  const entryAbility2 = getEntryAbility();
  if (entryAbility2) {
    return entryAbility2.context;
  }
...
// packages/uni-mp-sdk/src/UTSHarmony/convert2AbsFullPath.ts
function convert2AbsFullPath(filePath) {
...
// packages/uni-mp-sdk/src/UTSHarmony/getResourcePath.ts
...
// packages/uni-mp-sdk/src/UTSHarmony/getUserAgent.ts
function getUserAgent() {
  const mp = getCurrentMP();
  return mp && mp.userAgent.fullUserAgent;
}

// packages/uni-mp-sdk/src/UTSHarmony/appTheme.ts
var import_tiny_emitter7 = __toESM(require_tiny_emitter());
import dataPreferences from "@ohos.data.preferences";
var AppThemeStoreKey = "__APP_THEME";
var store4 = null;
function createStore() {
  if (store4 != null)
    return store4;
  const id = getCurrentMP().appId;
  store4 = dataPreferences.getPreferencesSync(getUIAbilityContext(), {
    name: `storage.${id}`
  });
  return store4;
}
var appThemeChangeEmitter = new import_tiny_emitter7.TinyEmitter();
var lastAppTheme = "light";
var isFirst2 = true;
nativePagesManager.onNativePageReady(() => {
  if (isFirst2) {
    isFirst2 = false;
    lastAppTheme = createStore().getSync(AppThemeStoreKey, "light");
    if (lastAppTheme === "auto") {
      osThemeChangeEmitter.on(ON_OS_THEME_CHANGE, AppThemeFollowsOsTheme);
    }
  }
});
function AppThemeFollowsOsTheme(res) {
  const osTheme = res.osTheme;
  appThemeChangeEmitter.emit(ON_APP_THEME_CHANGE, { appTheme: osTheme });
}
function setAppTheme(appTheme) {
  ...
  createStore().flush();
  if (isChange) {
    appThemeChangeEmitter.emit(ON_APP_THEME_CHANGE, { appTheme: appTheme === "auto" ? osTheme : appTheme });
  }
}
function getAppTheme() {
  return lastAppTheme;
}
var THEME_CALLBACKS2 = {};
var THEME_CALLBACK_ID2 = 0;
function onAppThemeChange(callback) {
  appThemeChangeEmitter.on(ON_APP_THEME_CHANGE, callback);
  const id = THEME_CALLBACK_ID2++;
  THEME_CALLBACKS2[id] = callback;
  return id;
}
function offAppThemeChange(id) {
  const callback = THEME_CALLBACKS2[id];
  if (id < 0 || typeof callback !== "function") {
    return;
  }
  delete THEME_CALLBACKS2[id];
  appThemeChangeEmitter.off(ON_APP_THEME_CHANGE, callback);
}

// packages/uni-mp-sdk/src/UTSHarmony/hostTheme.ts
var THEME_CALLBACKS3 = {};
var THEME_CALLBACK_ID3 = 0;
function onHostThemeChange(callback) {
  osThemeChangeEmitter.on(ON_OS_THEME_CHANGE, callback);
  const id = THEME_CALLBACK_ID3++;
  THEME_CALLBACKS3[id] = callback;
  return id;
}
function offHostThemeChange(id) {
  const callback = THEME_CALLBACKS3[id];
  if (id < 0 || typeof callback !== "function") {
    return;
  }
  delete THEME_CALLBACKS3[id];
  osThemeChangeEmitter.off(ON_OS_THEME_CHANGE, callback);
}

// packages/uni-mp-sdk/src/platform/getWindowInfo.ts
import window4 from "@ohos.window";
function getNativeSafeAreaInsets() {
  const mainWindow = getCurrentMP().window;
  const systemArea = mainWindow.getWindowAvoidArea(
    window4.AvoidAreaType.TYPE_SYSTEM
  );
  const navArea = mainWindow.getWindowAvoidArea(
    window4.AvoidAreaType.TYPE_NAVIGATION_INDICATOR
  );
...
function getNativeWindowRect() {
  const mainWindow = getCurrentMP().window;
  const windowProps = mainWindow.getWindowProperties();
  const screenRect = windowProps.windowRect;
  return {
...
function getWindowInfoWithoutPage() {
  const mainWindow = getCurrentMP()?.window;
  if (!mainWindow) {
    const safeAreaInsets2 = navigator.getSafeAreaInsets();
    return {
      safeAreaInsets: {
        top: safeAreaInsets2.top,
        bottom: safeAreaInsets2.bottom,
        left: safeAreaInsets2.left,
        right: safeAreaInsets2.right
      },
      safeArea: {
        top: safeAreaInsets2.top,
        bottom: 0,
      ...
      statusBarHeight: safeAreaInsets2.top,
      pixelRatio: vp2px(1),
      screenHeight: 0,
      screenTop: safeAreaInsets2.top,
      screenWidth: 0,
      windowHeight: 0,
      windowWidth: 0,
      windowTop: 0,
...
import dataPreferences2 from "@ohos.data.preferences";
var STORE_KEY_DEVICE_ID = "deviceId";
var SystemStorage = null;
function createSystemRootStorage() {
  if (!SystemStorage) {
    SystemStorage = dataPreferences2.getPreferencesSync(getAbilityContext(), {
...
import cryptoFramework2 from "@ohos.security.cryptoFramework";
import buffer5 from "@ohos.buffer";
function md5(str) {
  const md = cryptoFramework2.createMd("MD5");
  md.updateSync({
    data: new Uint8Array(buffer5.from(str, "utf8").buffer)
  });
  return buffer5.from(md.digestSync().data).toString("hex").toUpperCase();
}
...
var import_tiny_emitter8 = __toESM(require_tiny_emitter());
setup();
var export_Emitter = import_tiny_emitter8.TinyEmitter;
export {
