...
<script setup>
  ...
  function init(callback) {
    ...
    console.log("[uni-ec-canvas] start Canvas 2D initialization", props.canvasId);
  ...
  function initByNewWay(callback) {
        ...
        query
          ...
          .exec(res => {
            console.log("[uni-ec-canvas] canvas query result", res);
            if (!res || !res[0] || !res[0].node) {
              console.error("[uni-ec-canvas] Canvas 2D node is unavailable", res);
              return;
            }
            ...
            console.log("[uni-ec-canvas] canvas size", canvasWidth, canvasHeight, canvasDpr);
            if (canvasWidth <= 0 || canvasHeight <= 0) {
              console.error("[uni-ec-canvas] Canvas has no drawable size");
              return;
            }

            const ctx = canvasNode.getContext("2d");
            if (!ctx) {
              console.error("[uni-ec-canvas] Canvas 2D context is unavailable");
              return;
            }
  ...
  function initChart(canvas, width, height, canvasDpr) {
        ...
        console.log("[uni-ec-canvas] ECharts option applied", width, height);
